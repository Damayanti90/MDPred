...
count=0
c=0
fs=open(r"result_drug_go_prediction_data_XGBoost.txt","w")
...
print("\nGlobal SHAP for drug-GO")
print(imp.to_string(index=False), "\n")
with open('feature_importance_drug_go_XGBoost.txt', 'w') as f:
...
plt.savefig('drug_GO_beeswarm_plot_XGBoost.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()
...
fet_names = [str(i) for i in range(1,contrib.shape[1]+1)]
imp = pd.DataFrame({'feature': fet_names,'importance': mean_abs,'percentage': percent}).sort_values(by='percentage', ascending=False)
print("\nGlobal SHAP for drug-pathway")
print(imp.to_string(index=False), "\n")
with open('feature_importance_drug_pathway_XGBoost.txt', 'w') as f:
    f.write(imp.to_string(index=False))
plt.figure()
shap.summary_plot(contrib, X_test, feature_names=fet_names, show=False)
plt.title("Global SHAP summary for drug-pathway")
plt.savefig('drug_pathway_beeswarm_plot_XGBoost.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()
