...
fs=open(r"trial_info_lightGBM.txt","w")
...
for trial in study.trials:
    ...
    s+="learning_rate="+str(trial.params['learning_rate'])+"`"
    s+="num_leaves="+str(trial.params['num_leaves'])+"`"
    s+="max_depth="+str(trial.params['max_depth'])+"`"
    s+="min_child_samples="+str(trial.params['min_child_samples'])+"`"
...
count=0
c=0
fs=open(r"result_test_data.txt","w")
...
symb="`"
count=0
ft=open(r"drug_go_prediction_data.txt","r")
#ft=open(r"protein_pathway_test.txt","r")
pred_lst=[]
for line in ft:
...
count=0
c=0
fs=open(r"result_drug_go_prediction_data_lightGBM.txt","w")
#fs=open(r"result_test_protein_go_1.txt","w")
for i in range(len(pred_lst)):
    count+=1
    s=""
...
symb="`"
count=0
ft=open(r"drug_pathway_prediction_data.txt","r")
#ft=open(r"protein_pathway_test.txt","r")
pred_lst=[]
for line in ft:
    x=line.split(symb)
...
count=0
c=0
fs=open(r"result_drug_pathway_prediction_data_lightGBM.txt","w")
#fs=open(r"result_test_protein_go_1.txt","w")
for i in range(len(pred_lst)):
    count+=1
    s=""
...
symb="`"
count=0
ft=open(r"test_data_drug_go.txt","r")
pred_lst=[]
for line in ft:
    count+=1
    #if count>100:
...
with open('feature_importance_drug_GO_lightGBM.txt', 'w') as f:
    f.write(imp.to_string(index=False))


plt.figure(figsize=(10, 6))
shap.summary_plot(val, X_test, feature_names=fet_names, show=False)
plt.title("Global SHAP Summary for drug-GO")
plt.savefig('drug_GO_beeswarm_plot_lightGBM.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()
...
symb="`"
count=0
ft=open(r"test_data_drug_pathway.txt","r")
pred_lst=[]
for line in ft:
    x=line.split(symb)
    X_test.append(node_dic[x[0]]+node_dic[x[1]])
...
print("\n Global SHAP Feature Importance")
print(imp.to_string(index=False), "\n")
with open('feature_importance_drug_pathway_lightGBM.txt', 'w') as f:
    f.write(imp.to_string(index=False))


plt.figure(figsize=(10, 6))
shap.summary_plot(val, X_test, feature_names=fet_names, show=False)
plt.title("Global SHAP Summary for drug-pathway")
plt.savefig('drug_pathway_beeswarm_plot_lightGBM.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()
